...
3. **Verify locally**
   ...
   database has no projects), and GitHub / Linear / Jira smokes that **skip** when
   `GITHUB_TOKEN` / `GITHUB_REPO`, `LINEAR_API_KEY`, or `JIRA_*` credentials are unset
   (so CI does not need those secrets), and `mcp:pmgo:check` (imports `mcp` + `pyyaml` when installed;
